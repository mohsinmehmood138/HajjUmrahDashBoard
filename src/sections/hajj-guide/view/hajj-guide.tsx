...
import AddHajjGuide from '../add-hajj-guide';
import {
  deleteDocument,
  deleteItemFromArray,
  getFromCollection,
...
export function HajjGuideView() {
  ...
  useEffect(() => {
    const fetchDauData = async () => {
      setLoading(true);
      const data: any = await getFromCollection('hajj_guide');
    ...
    try {
      ...
      if (selectedIds.length > 0) {
        const results = await Promise.all(
          selectedIds.map((id) => deleteDocument('hajj_guide', id.toString()))
        );

        if (results.every((result) => result.success)) {
          toast.success('Hajj Guide Delete Successfully!', {
            position: 'top-center',
            theme: 'colored',
          });
          setFetchData(true);
      ...
      if (subListId) {
        result = await deleteItemFromArray('hajj_guide', menuRowId, 'items', subListId);
      } else {
        result = await deleteDocument('hajj_guide', menuRowId);
      }
